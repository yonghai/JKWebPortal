...
public class AddShops extends HttpServlet {
	private Logger logger = Logger.getLogger("jk.o1office.ht.operate.servlet.AddShops");
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		//	shops.setShopimg(fileUpload.getPath().replace("e:\\jike\\", ""));	//windows
			logger.info("imapath="+fileUpload.getPath());
			shops.setShopimg(fileUpload.getPath().replace("/jike/", ""));	//linux
			returninfo = shopsService.addShops(shops);
		} catch (Exception e) {
			returninfo = "添加商家失败";
