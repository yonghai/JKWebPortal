...
public class XMLUtils {
	...
	private synchronized static Document createOrUpdateXML(String filename) throws Exception{
	...
	public synchronized static void write(Document document, String filename) throws IOException{
		//输出全部原始数据，在编译器中显示  
        ...
        XMLWriter writer2 = new XMLWriter(new FileWriter(new File(  
        		FILEPATH+DateUtil.getCurrentTime("yyyyMMdd")+"_"+filename+".xml")), format);  
	...
	private static boolean isHas(String filename){
		File file = new File(FILEPATH+DateUtil.getCurrentTime("yyyyMMdd")+"_"+filename+".xml");
		if(file.exists())
			return true;
		return false;
	}
	 ...
	 * 
	 */
	public synchronized static void addOperate(OperateType operateType, FileType fileType, String token, String status){
		try {
			...
			operatetime.setText(DateUtil.getCurrentTime("yyyyMMdd"));
	...
	private static Document getDocument(String filename) throws DocumentException{
        ...
        Document xmlDoc = sax.read(new File(FILEPATH+DateUtil.getCurrentTime("yyyyMMdd")+"_"+filename+".xml"));  
        return xmlDoc;
	}
