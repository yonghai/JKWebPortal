...
<html>
  <head>
	...
	var localid;
	 ...
	 function setValue(){
		...
		//  $("#shengselect option:selected").val(),
		//  $("#shiselect option:selected").val(),
		//  $("#areaselect option:selected").val(),
		/*  $("#shengselect option").each(function(){
		  	if($(this).val()==row.sheng){
				$(this).attr("selected",true);
			}
		  });
		  $("#shiselect option").each(function(){
			  	alert($(this).val())
			  	if($(this).val()==row.shi){
					$(this).attr("selected",true);
				}
		  ...
		  $("#areaselect option").each(function(){
			  	if($(this).val()==row.area){
					$(this).attr("selected",true);
				}
		  });*/
		  localid = row.id;
		  $("input[name='shopname']").val(row.shopname);
	 	  $("input[name='bossname']").val(row.bossname);
		  $("input[name='bosstel']").val(row.bosstel);
		  $("input[name='shoptel']").val(row.shoptel);
		  $("input[name='shopaddr']").val(row.shopaddr);
		//  $("input[name='fileToUpload']").val(row.shopimg);
	 }
  ...
  <body>
  		<center>
		<div class="easyui-panel" title="编辑商家" style="width:600px">
