...
public class DeliveryUserDaoImpl implements UserDao {
	...
	public Object getUserByID(int userID) {
		List user = hibernateTemplate.find("from DeliveryUser au where au.id="+userID);
		if(user.size()!=0)
			return user.get(0);
		return null;
	}
	...
	public String getUserTelByID(int userID) {
		List<String> tel = hibernateTemplate.find("select du.telphone from DeliveryUser du where du.id="+userID);
		if(tel.size()>0)
			return tel.get(0);
		return "";
	}
