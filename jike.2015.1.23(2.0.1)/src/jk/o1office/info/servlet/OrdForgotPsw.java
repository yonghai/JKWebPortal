 ...
 *	参数已进行验证
 */
public class OrdForgotPsw extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if(session.getAttr(cuser.getUsername()).equals(checkcode)){
				returninfo = userService.updatePsw(cuser,token);
				MyLog.newInstance().writeLog(OperateType.UFORGETPASSWORD, FileType.UUSERINFO, token, "success");
			}else{
				returninfo = "{\"success\":false,\"token\":\""+token+"\",\"emsg\":\"验证码错误\"}";
				MyLog.newInstance().writeLog(OperateType.UFORGETPASSWORD, FileType.UUSERINFO, token, "fail");
			}
