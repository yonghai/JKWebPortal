...
public class UpdateNickName extends HttpServlet{
	private Logger logger = Logger.getLogger("jk.o1office.info.servlet.UpdateNickName");
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			String nickName = validator.isNull(req.getParameter("nickname"), "nickname");
			returninfo = userService.updateNickName(nickName,token);
			MyLog.newInstance().writeLog(OperateType.UUPDATENICKNAME, FileType.UUSERINFO, token, "success");
		} catch (NullException e) {
			returninfo = "{\"success\":false,\"token\":\""+token+"\",\"emsg\":\""+validator.getName()+"不能为null\"}";
			MyLog.newInstance().writeLog(OperateType.UUPDATENICKNAME, FileType.UUSERINFO, token, "fail");
			e.printStackTrace();
			...
			returninfo = "{\"success\":false,\"token\":\""+token+"\",\"emsg\":\"修改昵称失败\"}";
			MyLog.newInstance().writeLog(OperateType.UUPDATENICKNAME, FileType.UUSERINFO, token, "fail");
			e.printStackTrace();
			e.printStackTrace(ExceptionInfo.out());
			ExceptionInfo.out().flush();
