...
<html>
	...
	<script type="text/javascript" charset="utf-8">
	 ...
	 function edituser(value,rows,index){
		rows = JSON.stringify(rows);
	 	return "<a href='javascript:void(0)' onclick='editdeliveryadmin("+rows+")'>编辑</a>";
	 }
	 ...
	 function editdeliveryadmin(rows){
		 $("#deliverywin input[name='id']").val(rows.id);
		 $("#deliverywin div input[name='username']").val(rows.username);
		 $("#deliverywin div input[name='nickname']").val(rows.nickname);
		 $("#deliverywin div input[name='password']").val(rows.password);
		 $('#deliverywin').window({  
			 	title:'管理员信息',  
			    width:300,    
			    height:260,    
	 ...
	 function forgive(){
		...
		function update(){
			$.ajax({
				...
				data:{
					id:$("#deliverywin input[name='id']").val(),
					username:$("#deliverywin div input[name='username']").val(),
					nickname:$("#deliverywin div input[name='nickname']").val(),
					password:$("#deliverywin div input[name='password']").val(),
					type:'0'
				},
  ...
  <body>
		...
		<!-- 配送员的信息(弹出框) -->
		<div id="deliverywin" style="display: none;">
