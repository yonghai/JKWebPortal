...
public class ReceiveBillDaoImpl implements ReceiveBillDao {
	...
	public List<ReceiveBillH> getReceiveBill(final int pagesize,final int offset, final String time, final int userID) {
		...
		return hibernateTemplate.executeFind(new HibernateCallback<List<ReceiveBillH>>() {
			public List<ReceiveBillH> doInHibernate(Session session)
				...
				return session.createSQLQuery("select d.orderid,d.createtime,d.status,b.total_price,b.deliveryprice from dreceviebill d,bill b where d.deliveryid="+userID+
						" and d.orderid=b.orderid group by d.createtime desc")
			...
			return hibernateTemplate.executeFind(new HibernateCallback<List<ReceiveBillH>>() {
				public List<ReceiveBillH> doInHibernate(Session session)
						throws HibernateException, SQLException {
					return session.createSQLQuery("select d.orderid,d.createtime,d.status,b.total_price from dreceivebill_history d,bill b where d.deliveryid="+userID+
	...
	@SuppressWarnings("unchecked")
	public List<ReceiveBillItemH> getReceiveBDec(String orderid, int userID) {
		return hibernateTemplate.find("from ReceiveBillItemH rbih where rbih.billid="+orderid);
	}
	
	public void saveSubmitBill(String orderid, int userid) {
	...
	public String getTodayPrice(int userid) {
		String sql = "select sum(total_price) from bill where delivery_id="+userid+" and bill_status=2 and to_days(createtime)=to_days(now())";
